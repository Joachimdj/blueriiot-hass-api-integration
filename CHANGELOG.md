...
### Added
- Custom Lovelace card (`blueriot-pool-card`) — auto-registers on HA startup, no manual resource configuration needed
- Card shows primary metrics (pH, temperature, ORP) prominently, all other measurements in a responsive grid
- Color-coded indicators: green = in range, orange = warning, red = out of range
- Displays last measurement timestamp
